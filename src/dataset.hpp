...
struct Dataset {
    ...
    Dataset(DatasetType type, TaskType task, const Matrix& features, const Matrix& labels, int train_samples = 0)
        : type(type), task(task), features(features), labels(labels), train_samples(train_samples) {
...
struct DataSplit {
    ...
    static std::vector<DataSplit> split(int k, Scalar train_ratio, int train_samples, int num_samples, bool shuffle);
};