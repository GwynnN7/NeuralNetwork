...
Matrix load_mnist_labels(const std::string& path, Scalar dataset_ratio) {
    ...
    // Convert the raw labels to one-hot encoded format
...
Dataset Dataset::load(DatasetType dataset_type, Scalar dataset_ratio) {
    ...
    case DatasetType::XOR: {
        ...
        // Split the data into features and labels (last row is labels)
    ...
    case DatasetType::XOR_HOT: {
        ...
        // Split the data into features and labels (last two rows are labels)
        Matrix features = xor_hot_data.topRows(xor_hot_data.rows() - 2);
        Matrix labels = xor_hot_data.bottomRows(2);

        // Replicate the dataset to create a larger dataset for training and testing (task set to classification for XOR_HOT)
    ...
    case DatasetType::MNIST: {
        ...
        // Unify datasets to use personal k-fold cross-validation (use --inner-k 0 and default args to use the original train/test split)
